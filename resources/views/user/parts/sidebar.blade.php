                    ...
                    <div x-show="open" @click="open = false; document.body.classList.remove('modal-open')"
                        ...
                        <div @click.stop class="bg-white w-1/2 h-1/2 p-5 rounded relative">
                            ...
                            <div class="">
                                ...
                                @foreach ($bastTypes as $value => $text)
                                    <!-- ラベル -->
                                    <label>
                                        <input name="cast_type[]" type="checkbox" value="{{ $value }}"
                                            {{ in_array($value, session('selectedItems', [])) ? 'checked' : '' }}>
                                        {{ $text }}
                                    </label>
                                @endforeach
                                <div class="">
                                    <h2 class="mt-2">バストタイプ</h2>
                                    @foreach ($bodyTypes as $value => $text)
                                        <!-- ラベル -->
                                        <label>
                                            <input name="cast_type[]" type="checkbox" value="{{ $value }}"
                                                {{ in_array($value, session('selectedItems', [])) ? 'checked' : '' }}>
                        ...
                        <div @click.stop class="bg-white w-3/4 h-1/2 p-5 rounded relative">
                            ...
                            <div class="">
                                ...
                                @foreach ($looks as $index => $value)
                                    <!-- チェックボックス -->
                                    <input name="cast_type[]" type="checkbox" value="{{ $index }}"
                                        :id="'{{ $value }}'" class="hidden-checkbox"
                        ...
                        <div @click.stop class="bg-white w-3/4 h-1/2 p-5 rounded relative">
                            ...
                            <div class="">
                                ...
                                @foreach ($bodyTypes as $index => $value)
                                    <!-- チェックボックス -->
                                    <input name="cast_play[]" type="checkbox" value="{{ $index }}"
                                        :id="'{{ $value }}'" class="hidden-checkbox"
                                        @change="updateSelected($event, '{{ $value }}')">
